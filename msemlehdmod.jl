...
function main()
	...
	Fin=5
	Tot=Fin-Ini+1
	Nex=[10^i for i=Ini:Fin]
	
	#ALTENCION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	#ALTENCION !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    NameArch="try.h5"
# 	NameArch="prueba"*string(Dim)*"It30.h5"
	...
	Grad=zeros(Complex{Float64},Dim)
   	...
   	for i=1:It 
		...
		@views GuessMi[:,i].=normalize(GuessIn[:,i]-Step) #Minus perturbation
        	...
        	@views FunPl[:,i].=4*SE(Psi,GuessPl[:,i],Nex) #Plus meditions
		@views FunMi[:,i].=4*SE(Psi,GuessMi[:,i],Nex) #Minus meditions
		#gradient estimation        
	    	@views Grad.=sum(FunPl[1:Dim,i]-FunMi[1:Dim,i])./(2*conj(Step)) 	
		@views GuessIn[:,i+1].=normalize(GuessIn[:,i]-ak*Grad) #next guess generation
		#Parmsametrizations
 		...
 		Res=Optim.minimizer(optimize(Target,Para,NelderMead())) #Optimization of MLE
     		GuessIn[:,i+1]=normalize(Res[1:Dim]+1im*Res[Dim+1:2*Dim]) #new guess
...
@everywhere function KLDgauss(DataPl,DataMi,GuessPl,GuessMi,Para,Dim,It) #Kullback-Leibler Divergence
	State=zeros(Complex{Float64},Dim,1)	
	@views State[:,1] .= normalize(Para[1:Dim]+1im*Para[Dim+1:2*Dim])  
	ProbPl=zeros(2*Dim,It)
	ProbMi=zeros(2*Dim,It) 
	...
	@views @. ProbMi[(Dim +1):end,:].=  abs2(State+GuessMi[:,1:It])
	@views @. ProbMi[1:Dim,:].= abs2(State-GuessMi[:,1:It])
    @views TheorProb=ProbPl[:,1:It]+ProbMi[:,1:It]
    @views StDev =sqrt.(TheorProb)
	#KL divergence (model dependent part)
    return @views sum(abs2.(sqrt(.5)*(DataPl[:,1:It]+DataMi[:,1:It]-TheorProb)./StDev))
end
